...
class TestTurnover(unittest.TestCase):
    ...
    def testDump(self):
        turnover = Turnover(name=self.TEST_TURNOVER['name'])
        turnover.setId(id=self.TEST_TURNOVER['id'])
        for d in self.TEST_TURNOVER['data']:
            turnover.append(d)
        self.assertEqual(self.TEST_TURNOVER, turnover.turnover)
